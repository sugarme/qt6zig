...
int QImage_Metric(const QImage* self, int metric) {
	...
	return self->QImage::metric(static_cast<QPaintDevice::PaintDeviceMetric>(metric));
}
}
...
QImage* QImage_ConvertToFormatHelper(const QImage* self, int format, int flags) {
	auto* vqimage = dynamic_cast<const VirtualQImage*>(self);
	if (vqimage && vqimage->isVirtualQImage) {
	return new QImage(vqimage->convertToFormat_helper(static_cast<QImage::Format>(format), static_cast<QFlags<Qt::ImageConversionFlag>>(flags)));
	} else {
	return new QImage(self->QImage::convertToFormat_helper(static_cast<QImage::Format>(format), static_cast<QFlags<Qt::ImageConversionFlag>>(flags)));
}
}
...
bool QImage_ConvertToFormatInplace(QImage* self, int format, int flags) {
	auto* vqimage = dynamic_cast<VirtualQImage*>(self);
	if (vqimage && vqimage->isVirtualQImage) {
	return vqimage->convertToFormat_inplace(static_cast<QImage::Format>(format), static_cast<QFlags<Qt::ImageConversionFlag>>(flags));
	} else {
	return self->QImage::convertToFormat_inplace(static_cast<QImage::Format>(format), static_cast<QFlags<Qt::ImageConversionFlag>>(flags));
}
}
...
QImage* QImage_SmoothScaled(const QImage* self, int w, int h) {
	auto* vqimage = dynamic_cast<const VirtualQImage*>(self);
	if (vqimage && vqimage->isVirtualQImage) {
	return new QImage(vqimage->smoothScaled(w, h));
	} else {
	return new QImage(self->QImage::smoothScaled(w, h));
}
}
