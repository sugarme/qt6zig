...
int QSpinBox_Validate(const QSpinBox* self, libqt_string input, int* pos) {
	...
	if (vqspinbox && vqspinbox->isVirtualQSpinBox) {
	return vqspinbox->validate(QString::fromUtf8(input.data, input.len), *pos);
	} else {
	return self->QSpinBox::validate(QString::fromUtf8(input.data, input.len), *pos);
}
}
...
int QSpinBox_ValueFromText(const QSpinBox* self, const libqt_string text) {
	auto* vqspinbox = dynamic_cast<const VirtualQSpinBox*>(self);
	if (vqspinbox && vqspinbox->isVirtualQSpinBox) {
	return vqspinbox->valueFromText(QString::fromUtf8(text.data, text.len));
	} else {
	return self->QSpinBox::valueFromText(QString::fromUtf8(text.data, text.len));
}
}
...
libqt_string QSpinBox_TextFromValue(const QSpinBox* self, int val) {
	auto* vqspinbox = dynamic_cast<const VirtualQSpinBox*>(self);
	if (vqspinbox && vqspinbox->isVirtualQSpinBox) {
	QString _ret = vqspinbox->textFromValue(val);
	QByteArray _b = _ret.toUtf8();
	libqt_string _str;
	_str.len = _b.length();
	_str.data = static_cast<const char*>(malloc(_str.len + 1));
	...
	QString _ret = self->QSpinBox::textFromValue(val);
	QByteArray _b = _ret.toUtf8();
	libqt_string _str;
	_str.len = _b.length();
	_str.data = static_cast<const char*>(malloc(_str.len + 1));
...
void QSpinBox_Fixup(const QSpinBox* self, libqt_string str) {
	auto* vqspinbox = dynamic_cast<const VirtualQSpinBox*>(self);
	if (vqspinbox && vqspinbox->isVirtualQSpinBox) {
	vqspinbox->fixup(QString::fromUtf8(str.data, str.len));
	} else {
	self->QSpinBox::fixup(QString::fromUtf8(str.data, str.len));
}
}
