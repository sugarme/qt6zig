...
bool QSqlTableModel_UpdateRowInTable(QSqlTableModel* self, int row, const QSqlRecord* values) {
	auto* vqsqltablemodel = dynamic_cast<VirtualQSqlTableModel*>(self);
	if (vqsqltablemodel && vqsqltablemodel->isVirtualQSqlTableModel) {
	return vqsqltablemodel->updateRowInTable(row, *values);
	} else {
	return self->QSqlTableModel::updateRowInTable(row, *values);
}
}
...
bool QSqlTableModel_DeleteRowFromTable(QSqlTableModel* self, int row) {
	auto* vqsqltablemodel = dynamic_cast<VirtualQSqlTableModel*>(self);
	if (vqsqltablemodel && vqsqltablemodel->isVirtualQSqlTableModel) {
	return vqsqltablemodel->deleteRowFromTable(row);
	} else {
	return self->QSqlTableModel::deleteRowFromTable(row);
}
}
...
QSqlRecord* QSqlTableModel_PrimaryValues(const QSqlTableModel* self, int row) {
	auto* vqsqltablemodel = dynamic_cast<const VirtualQSqlTableModel*>(self);
	if (vqsqltablemodel && vqsqltablemodel->isVirtualQSqlTableModel) {
	return new QSqlRecord(vqsqltablemodel->primaryValues(row));
	} else {
	return new QSqlRecord(self->QSqlTableModel::primaryValues(row));
}
}
